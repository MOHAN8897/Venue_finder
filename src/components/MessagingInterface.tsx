...
const ConversationListItem = React.memo(({ convo, activeConversation, onSelect }: { convo: typeof conversations[0], activeConversation: number, onSelect: (id: number) => void }) => (
  <div
...
const MessageBubble = React.memo(({ msg }: { msg: { from: 'me' | 'other'; text: string; timestamp: string } }) => (
  <div className={cn("flex items-end max-w-lg", msg.from === 'me' ? 'ml-auto flex-row-reverse' : 'mr-auto')}>
...
const MessagingInterface: React.FC = () => {
    ...
    const memoizedConversations = useMemo(() => conversations, []);
    const memoizedMessages = useMemo(() => currentMessages[activeConversation] || [], [currentMessages, activeConversation]);
    const handleSelectConversation = useCallback((id: number) => setActiveConversation(id), []);
        ...
        <Card className="h-[75vh] flex">
            ...
            <div className="w-1/3 border-r">
                ...
                <div className="p-2 space-y-1">
                    {memoizedConversations.map(convo => (
            ...
            <div className="w-2/3 flex flex-col">
                <CardHeader className="border-b">
                    <CardTitle>{memoizedConversations.find(c => c.id === activeConversation)?.name}</CardTitle>
                </CardHeader>
